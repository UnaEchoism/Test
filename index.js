(async function () {
    ...
    // ================= 极轻量 Entry / TauriTavern 适配 =================
    // TT 的聊天 DOM 采用虚拟化；播放器不参与聊天节点树，只在首次打开后创建独立 overlay。
    const isTauriTavern = !!window.__TAURITAVERN__;
    let targetDoc = document;
    let targetWin = window;

    // 标准 SillyTavern 保留原有 parent-document 兼容；TT 不跨到 parent，避免误挂载到宿主聊天树。
    if (!isTauriTavern) {
        try {
            if (window.parent && window.parent !== window && window.parent.document && window.parent.innerWidth > 0 && window.parent.innerHeight > 0) {
    ...
    // 启动阶段只有小对象，不读 localStorage，不解析歌单，不创建 Audio。
    let savedPlaylists = [{ id: 'default', name: '默认列表', description: '', cover: '', color: '', tracks: [] }];
    ...
    function dedupeAndLimitTracks(tracks) {
        ...
        for (const t of tracks || []) {
            if (result.length >= CONFIG.MAX_TRACKS_PER_LIST) break;
    ...
    function ensurePlayerDataLoaded() {
        ...
        try {
            ...
            if (raw) {
                ...
                if (Array.isArray(parsed)) {
                    savedPlaylists = (parsed.length > 0 && parsed[0]?.tracks)
                        ? parsed
                        : [{ id: 'default', name: '默认列表', description: '', cover: '', color: '', tracks: parsed }];
                }
            }
        } catch (e) { console.warn('[ApV] 读取本地歌单失败', e); }
        ...
        try {
            ...
            if (s) savedSettings = { ...DEFAULT_SETTINGS, ...JSON.parse(s) };
        } catch (_) { savedSettings = { ...DEFAULT_SETTINGS }; }
        delete savedSettings.customFont;

        STATE.playlists = savedPlaylists;
        STATE.currentPlaylistId = savedPlaylists[0].id;
        STATE.playingPlaylistId = savedPlaylists[0].id;
        STATE.currentTheme = savedSettings.theme || CONFIG.DEFAULT_THEME;
    }
    ...
    function preparePlaylistsForUI() {
        ...
        savedPlaylists.forEach(list => {
            ...
            if (removed > 0) {
                ...
                } else startupDedupeCount += removed;
            }
        });
        STATE.playlists = savedPlaylists;
        if (startupDedupeCount > 0 || startupLimitCount > 0) {
            try { localStorage.setItem(CONFIG.STORAGE_KEY, JSON.stringify(savedPlaylists)); } catch (_) {}
        }
    }
    ...
    const renderPlaylistCover = (container, playlist, className = '') => {
        ...
        if (playlist?.cover) {
            const img = targetDoc.createElement('img'); img.src = playlist.cover; img.alt = ''; img.draggable = false; container.appendChild(img);
        } else {
            const icon = targetDoc.createElement('i'); icon.className = playlist?.id === 'default' ? 'fas fa-music' : 'fas fa-compact-disc'; container.appendChild(icon);
        }
    ...
    // 保存操作只在用户发生歌单变更时触发，不参与宿主启动。
    const savePlaylist = () => {
        try {
            localStorage.setItem(CONFIG.STORAGE_KEY, JSON.stringify(STATE.playlists, (key, value) => key === '__retriedOnError' ? undefined : value));
        } catch (e) {
            console.warn('[ApV] 保存歌单失败', e);
            if (e.name === 'QuotaExceededError' || e.name === 'NS_ERROR_DOM_QUOTA_REACHED') API.toast('⚠️ 存储空间已满！请清理不必要的歌单或歌曲，以防脚本卡死。');
        }
    };
    ...
    function createPlayerRuntime() {
        ...
        audio = new targetWin.Audio();
        let runtimeSleeping = false;
        ...
        style.textContent = `
            ...
            * { box-sizing: border-box; font-family: var(--fm-font); margin: 0; padding: 0; }
            ...
            .fm-lrc-no-lyrics-title { font-size: var(--fm-lrc-font, 16px); font-weight: bold; color: var(--fm-accent); line-height: 1.4; text-shadow: 0 2px 8px var(--fm-shadow), 0 0 2px rgba(0,0,0,0.5); }
            .fm-lrc-no-lyrics-artist { margin-top: 3px; font-size: calc(var(--fm-lrc-font, 16px) * 0.72); color: var(--fm-text-sub); line-height: 1.35; text-shadow: 0 1px 4px var(--fm-shadow); }
        ...
        // 修复：壁纸自适应与边界限制
        function uiActive() {
            return !runtimeSleeping && STATE.isExpanded && !!UI?.panel?.isConnected;
        }

        function sleepPlayerUI() {
            runtimeSleeping = true;
            STATE.runtimeSleeping = true;
            STATE.isExpanded = false;
            STATE.isSeekingProgress = false;

            // 关键：真正把播放器主体从 DOM 树摘掉。
            // 桌面歌词节点留在独立的轻量宿主中继续工作；歌单、设置、按钮、进度条全部离开 DOM。
            try { UI.panel?.classList.remove('open'); } catch (_) {}
            try { UI.panel?.remove(); } catch (_) {}
            try { UI.ball?.classList.remove('playing'); } catch (_) {}

            // 关闭时把短期设置保存任务一次性结算，避免 SLEEP 期间还有播放器 timer。
        try { clearTimeout(settingsSaveTimer); saveSettings(); } catch (_) {}

        // 如果用户在拖拽/拖动进度时关闭，确保文档级监听被清掉。
            try { targetDoc.removeEventListener('pointermove', window.__apvDragMove); } catch (_) {}
            try { targetDoc.removeEventListener('pointerup', window.__apvDragUp); } catch (_) {}
            try { targetDoc.removeEventListener('pointercancel', window.__apvDragUp); } catch (_) {}
            window.__apvDragMove = null;
            window.__apvDragUp = null;
        }

        function wakePlayerUI() {
            if (!runtimeSleeping) return;
            runtimeSleeping = false;
            STATE.runtimeSleeping = false;
            if (UI.panel && !UI.panel.isConnected) UI.wrapper.insertBefore(UI.panel, UI.wrapper.querySelector('.fm-pop-menu') || UI.outLyrics || null);
            // 恢复后再进行一次完整 UI 同步；这一步只在用户主动打开播放器时发生。
            applySettings();
            if (uiActive()) {
                if (STATE.isShowingSearch || STATE.isPlaylistHome || STATE.currentPlaylistId) renderListUI();
                if (STATE.isPlaying) {
                    if (UI.playBtn) UI.playBtn.innerHTML = '<i class="fas fa-pause"></i>';
                    if (UI.ball) UI.ball.classList.add('playing');
        ...
        function togglePanel() {
            if (runtimeSleeping) wakePlayerUI();
            const currentSize = parseInt(savedSettings.ballSize) || 50;

            if (!STATE.isExpanded) {
                STATE.isExpanded = true;
                if (UI.panel && !UI.panel.isConnected) {
                    UI.wrapper.insertBefore(UI.panel, UI.wrapper.querySelector('.fm-pop-menu') || UI.outLyrics || null);
                }
                ...
                // 关闭 = 进入真正 SLEEP，而不是 display:none。
                sleepPlayerUI();
                UI.ball.innerHTML = '<i class="fas fa-music"></i>';
                if (savedSettings.showBall !== false && STATE.isPlaying) UI.ball.classList.add('playing');
            }
        }
        ...
        async function playTrack(index, playlistId = STATE.playingPlaylistId, isRetry = false) {
            ...
            if (uiActive()) updateProgressUI(0, 0);

            if (!track.url) {
                if (uiActive()) UI.title.textContent = "解析音源中...";
            ...
            if (track.url) {
                if (uiActive()) UI.title.textContent = track.title;
                ...
                if (track.lrcUrl || track.lyricId || track.rawId) {
                    ...
                    } else if (STATE.isLyricsVisible && !audio.paused) {
                        // 歌词接口明确没有返回歌词时，保留桌面“正在播放”状态。
                        updateLyrics();
                    }
                }
            } else {
                if (uiActive()) UI.title.textContent = "音源不可用，跳过";
        ...
        // 无歌词歌曲也要有桌面歌词状态：显示当前歌曲名/歌手，避免播放时桌面歌词完全消失。
        function renderNoLyricsDesktop() {
            if (!STATE.isLyricsVisible || !audio || audio.paused) return;
            const list = STATE.playlists.find(p => p.id === STATE.playingPlaylistId);
            const track = list && list.tracks[STATE.currentIndex];
            const title = track?.title || '正在播放';
            const artist = track?.artist || '暂无歌词';

            if (savedSettings.lrcMode === 'scroll') {
                if (!UI.outLyricsScrollList) return;
                UI.outLyricsScrollList.replaceChildren();
                const titleEl = targetDoc.createElement('div');
                titleEl.className = 'fm-lrc-scroll-line current';
                titleEl.textContent = `♪ ${title}`;
                const artistEl = targetDoc.createElement('div');
                artistEl.className = 'fm-lrc-scroll-line near';
                artistEl.textContent = artist;
                UI.outLyricsScrollList.append(titleEl, artistEl);
                UI.outLyricsScrollList.style.transform = 'translateY(0px)';
            } else {
                if (!UI.outLyrics) return;
                UI.outLyrics.replaceChildren();
                const titleEl = targetDoc.createElement('div');
                titleEl.className = 'fm-lrc-no-lyrics-title';
                titleEl.textContent = `♪ ${title}`;
                const artistEl = targetDoc.createElement('div');
                artistEl.className = 'fm-lrc-no-lyrics-artist';
                artistEl.textContent = artist;
                UI.outLyrics.append(titleEl, artistEl);
            }
        }
        ...
        function updateLyrics() {
            ...
            // 没有歌词时，桌面歌词仍显示当前正在播放的歌曲，而不是整块消失。
            if (STATE.lyricsData.length === 0) {
                renderNoLyricsDesktop();
                return;
            }
        ...
        audio.onplay = () => {
            ...
            if (uiActive()) {
                ...
                if (savedSettings.showBall !== false) UI.ball.classList.add('playing');
            }
            if (lrcRafId) cancelAnimationFrame(lrcRafId);
            updateLyrics();
        };
        audio.onpause = () => {
            ...
            if (UI.playBtn && uiActive()) UI.playBtn.innerHTML = '<i class="fas fa-play"></i>';
            if (UI.ball) UI.ball.classList.remove('playing');
            if (lrcRafId) cancelAnimationFrame(lrcRafId);
        };
        audio.onended = () => {
        ...
        audio.ontimeupdate = () => {
            // 播放核心与桌面歌词继续工作；进度条/播放器 UI 仅在 ACTIVE 时更新。
            if (uiActive() && !STATE.isSeekingProgress) updateProgressUI(audio.currentTime, audio.duration);
            if (STATE.isLyricsVisible) updateLyrics();
        };
        ...
        // 首次创建完成后立即进入 SLEEP：不让面板参与 TT 页面布局/虚拟化；桌面歌词仍可运行。
        sleepPlayerUI();
        ...
        return {
            ...
            close() {
                if (STATE.isExpanded) togglePanel();
                else if (!runtimeSleeping) sleepPlayerUI();
            },
            toggle() {
                if (STATE.isExpanded) togglePanel();
                else {
            ...
            toggleBallVisibility() {
                ...
                applySettings();
                if (savedSettings.showBall === false) UI.ball.classList.remove('playing');
                else if (STATE.isPlaying && runtimeSleeping === false) UI.ball.classList.add('playing');
            },
            sleep() { if (STATE.isExpanded) togglePanel(); else sleepPlayerUI(); },
            ...
            destroy() {
                ...
                try { targetDoc.getElementById(CONFIG.ID)?.remove(); } catch (_) {}
                audio = null;
    ...
    async function loadRuntime() {
        ...
        if (!runtimePromise) {
            runtimePromise = Promise.resolve().then(() => {
                // TT 宿主 ready 只在真正打开播放器时等待；启动阶段不碰 TT API。
                if (targetWin.__TAURITAVERN__) {
                    const ready = targetWin.__TAURITAVERN__.ready || targetWin.__TAURITAVERN_MAIN_READY__;
                    if (ready && typeof ready.then === 'function') return ready;
                }
                return null;
            }).then(() => {
                runtime = createPlayerRuntime();
                targetWin.__APV_PLAYER_RUNTIME__ = runtime;
                return runtime;
            }).catch(err => {
                runtimePromise = null;
    ...
    async function openPlayer() {
        try {
            ...
            try { if (typeof triggerSlash === 'function') triggerSlash('/echo severity=error [播放器测试] 播放器打开失败，请查看控制台。'); } catch (_) {}
        }
    }
    ...
    function installSillyTavernWandButton() {
        ...
        const old = doc.getElementById(MENU_ID);
        if (old) old.remove();
        ...
        item.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); openPlayer(); });
        wrap.appendChild(item);
        menu.appendChild(wrap);
        return true;
    }

    // TT 官方建议对 overlay 显式声明 surface；这里不依赖全局 subtree observer。
    function markTauriSurface(container) {
        if (isTauriTavern && container) container.setAttribute('data-tt-mobile-surface', 'FreeWindow');
    }

    // 极少量入口重试：只寻找酒馆扩展菜单，不创建播放器 DOM、不读存储。
    if (!installSillyTavernWandButton()) {
        let attempts = 0;
        const retry = () => {
            if (installSillyTavernWandButton() || ++attempts >= 4) return;
            targetWin.setTimeout(retry, 900);
        };
        targetWin.setTimeout(retry, 250);
    }

    targetWin._flowMusicToggle = async () => {
        try {
            const rt = await loadRuntime();
            rt.toggleBallVisibility();
        } catch (_) {}
    };
    if (typeof eventOn === 'function' && typeof getButtonEvent === 'function') {
        try { eventOn(getButtonEvent('显隐播放器'), targetWin._flowMusicToggle); } catch (_) {}
    }

    targetWin.addEventListener('pagehide', () => {
        try { runtime?.destroy?.(); } catch (_) {}
        try { targetWin.getElementById?.(CONFIG.ID)?.remove?.(); } catch (_) {}
        try { targetWin.document?.getElementById(CONFIG.ID)?.remove(); } catch (_) {}
        try { targetWin._flowMusicToggle = null; delete targetWin._flowMusicToggle; } catch (_) {}
        runtime = null;
        runtimePromise = null;
    }, { once: true });

    // 仅在首次真正创建播放器时给 overlay 标记；若用户从未打开，TT 页面完全没有播放器 DOM。
})();
