(async function () {
    ...
    const getSavedLrcFont = () => {
        ...
        return {
            id: savedSettings.lrcFontId || '',
            name: savedSettings.lrcFontName,
            family: savedSettings.lrcFontFamily,
            css: savedSettings.lrcFontCss,
    ...
    function togglePanel() {
        ...
        if (STATE.isExpanded) {
            if (typeof setPlayerRuntimeSleeping === 'function') setPlayerRuntimeSleeping(false);
            ...
            if (typeof setPlayerRuntimeSleeping === 'function') setPlayerRuntimeSleeping(true);
    ...
    audio.onplay = () => {
        ...
        if (!runtimeSleeping) UI.playBtn.innerHTML = '<i class="fas fa-pause"></i>';
        if (!runtimeSleeping) UI.ball.classList.add('playing');
        if (lrcRafId) cancelAnimationFrame(lrcRafId);
        updateLyrics();
    };
    audio.onpause = () => {
        STATE.isPlaying = false;
        if (!runtimeSleeping) UI.playBtn.innerHTML = '<i class="fas fa-play"></i>';
        if (!runtimeSleeping) UI.ball.classList.remove('playing');
        if (lrcRafId) cancelAnimationFrame(lrcRafId);
    };
    audio.onended = () => {
    ...
    audio.ontimeupdate = () => {
        if (!runtimeSleeping && !STATE.isSeekingProgress) updateProgressUI(audio.currentTime, audio.duration);
        if (STATE.isLyricsVisible) updateLyrics();
    };
    audio.onloadedmetadata = () => { if (!runtimeSleeping && !STATE.isSeekingProgress) updateProgressUI(audio.currentTime, audio.duration); };
    audio.onerror = () => {
    ...
    // ================= 运行时生命周期 =================
    // 这个文件由轻量 Entry 在第一次打开播放器时动态加载。
    // 因此这里不再安装酒馆菜单、不注册全局显隐事件、不自动打开播放器。
    let runtimeSleeping = true;

    function setPlayerRuntimeSleeping(sleeping) {
        runtimeSleeping = !!sleeping;
        STATE.runtimeSleeping = runtimeSleeping;
        if (runtimeSleeping) {
            // 只保留音频播放与桌面歌词运行；播放器 UI 完全停止刷新。
            if (UI.panel) UI.panel.classList.remove('open');
            if (UI.playBtn) UI.playBtn.onclick = null;
            if (UI.prevBtn) UI.prevBtn.onclick = null;
            if (UI.nextBtn) UI.nextBtn.onclick = null;
            if (UI.modeBtn) UI.modeBtn.onclick = null;
            if (UI.lrcToggleBtn) UI.lrcToggleBtn.onclick = null;
            if (UI.lrcSettingsBtn) UI.lrcSettingsBtn.onclick = null;
            if (UI.lrcModePlainBtn) UI.lrcModePlainBtn.onclick = null;
            if (UI.lrcModePopupBtn) UI.lrcModePopupBtn.onclick = null;
            if (UI.lrcModeScrollBtn) UI.lrcModeScrollBtn.onclick = null;
            if (UI.lrcModeFallBtn) UI.lrcModeFallBtn.onclick = null;
            if (UI.lrcFontSlider) UI.lrcFontSlider.oninput = null;
            if (UI.lrcBottomSlider) UI.lrcBottomSlider.oninput = null;
            if (UI.sourceSelect) UI.sourceSelect.onchange = null;
            if (UI.addBtn) UI.addBtn.onclick = null;
            if (UI.input) UI.input.oninput = null;
            if (UI.shapeBtn) UI.shapeBtn.onclick = null;
            if (UI.sizeSlider) UI.sizeSlider.oninput = null;
            if (UI.ballVisibleToggle) UI.ballVisibleToggle.onchange = null;
            if (UI.colorPicker) UI.colorPicker.oninput = null;
            if (UI.decorationUploadBtn) UI.decorationUploadBtn.onclick = null;
            if (UI.decorationUploadInput) UI.decorationUploadInput.onchange = null;
            if (UI.decorationClearBtn) UI.decorationClearBtn.onclick = null;
            if (UI.decorationLabelInput) UI.decorationLabelInput.oninput = null;
            if (UI.bgUploadBtn) UI.bgUploadBtn.onclick = null;
            if (UI.bgUploadInput) UI.bgUploadInput.onchange = null;
            if (UI.bgClearBtn) UI.bgClearBtn.onclick = null;
            if (UI.ratioBtn) UI.ratioBtn.onclick = null;
            if (UI.bgBlurSlider) UI.bgBlurSlider.oninput = null;
            if (UI.bgBrightnessSlider) UI.bgBrightnessSlider.oninput = null;
            if (UI.closeBtn) UI.closeBtn.onclick = null;
            if (UI.lrcFontImport) UI.lrcFontImport.onclick = null;
            if (UI.lrcFontUrl) UI.lrcFontUrl.onkeydown = null;
            if (UI.lrcFontReset) UI.lrcFontReset.onclick = null;
            pageEls.forEach(p => p.style.pointerEvents = 'none');
            navEls.forEach(b => b.onclick = null);
            if (quickLibraryBtn) quickLibraryBtn.onclick = null;
            // 桌面歌词仍由 audio 时间轴驱动，因此不要取消 lrcRaf。
        } else {
            pageEls.forEach(p => p.style.pointerEvents = '');
            bindRuntimeUI();
            applySettings();
            renderListUI();
            if (STATE.isPlaying) updateLyrics();
        }
    }

    function bindRuntimeUI() {
        if (UI.closeBtn) UI.closeBtn.onclick = originalCloseHandler || togglePanel;
        if (UI.playBtn) UI.playBtn.onclick = originalPlayHandler;
        if (UI.prevBtn) UI.prevBtn.onclick = originalPrevHandler;
        if (UI.nextBtn) UI.nextBtn.onclick = originalNextHandler;
        if (UI.modeBtn) UI.modeBtn.onclick = originalModeHandler;
        if (UI.lrcToggleBtn) UI.lrcToggleBtn.onclick = originalLrcToggleHandler;
        if (UI.lrcSettingsBtn) UI.lrcSettingsBtn.onclick = originalLrcSettingsHandler;
        if (UI.lrcModePlainBtn) UI.lrcModePlainBtn.onclick = originalLrcPlainHandler;
        if (UI.lrcModePopupBtn) UI.lrcModePopupBtn.onclick = originalLrcPopupHandler;
        if (UI.lrcModeScrollBtn) UI.lrcModeScrollBtn.onclick = originalLrcScrollHandler;
        if (UI.lrcModeFallBtn) UI.lrcModeFallBtn.onclick = originalLrcFallHandler;
        if (UI.lrcFontSlider) UI.lrcFontSlider.oninput = originalLrcFontSliderHandler;
        if (UI.lrcBottomSlider) UI.lrcBottomSlider.oninput = originalLrcBottomHandler;
        if (UI.sourceSelect) UI.sourceSelect.onchange = originalSourceHandler;
        if (UI.addBtn) UI.addBtn.onclick = originalAddButtonHandler;
        if (UI.input) UI.input.oninput = originalInputHandler;
        if (UI.shapeBtn) UI.shapeBtn.onclick = originalShapeHandler;
        if (UI.sizeSlider) UI.sizeSlider.oninput = originalSizeHandler;
        if (UI.ballVisibleToggle) UI.ballVisibleToggle.onchange = originalBallVisibleHandler;
        if (UI.colorPicker) UI.colorPicker.oninput = originalColorHandler;
        if (UI.decorationUploadBtn) UI.decorationUploadBtn.onclick = () => UI.decorationUploadInput?.click();
        if (UI.decorationUploadInput) UI.decorationUploadInput.onchange = originalDecorationUploadHandler;
        if (UI.decorationClearBtn) UI.decorationClearBtn.onclick = originalDecorationClearHandler;
        if (UI.decorationLabelInput) UI.decorationLabelInput.oninput = originalDecorationLabelHandler;
        if (UI.bgUploadBtn) UI.bgUploadBtn.onclick = () => UI.bgUploadInput?.click();
        if (UI.bgUploadInput) UI.bgUploadInput.onchange = originalBgUploadHandler;
        if (UI.bgClearBtn) UI.bgClearBtn.onclick = originalBgClearHandler;
        if (UI.ratioBtn) UI.ratioBtn.onclick = originalRatioHandler;
        if (UI.bgBlurSlider) UI.bgBlurSlider.oninput = originalBgBlurHandler;
        if (UI.bgBrightnessSlider) UI.bgBrightnessSlider.oninput = originalBgBrightnessHandler;
        if (UI.lrcFontImport) UI.lrcFontImport.onclick = guardedImportZeoSevenFont;
        if (UI.lrcFontUrl) UI.lrcFontUrl.onkeydown = e => { if (e.key === 'Enter') guardedImportZeoSevenFont(); };
        if (UI.lrcFontReset) UI.lrcFontReset.onclick = resetLrcFontToDefault;
        navEls.forEach(btn => btn.onclick = () => switchPlayerPage(btn.dataset.pageTarget));
        if (quickLibraryBtn) quickLibraryBtn.onclick = () => { switchPlayerPage('play'); UI.playlistSection?.scrollIntoView({behavior:'smooth',block:'start'}); };
    }

    // 兼容旧代码中在文件后半段定义的静态事件处理器。
    // 这些变量由下面的补丁初始化为当前元素已有 handler，再由 bindRuntimeUI 恢复。
    const originalCloseHandler = UI.closeBtn?.onclick || null;
    const originalPlayHandler = UI.playBtn?.onclick || null;
    const originalPrevHandler = UI.prevBtn?.onclick || null;
    const originalNextHandler = UI.nextBtn?.onclick || null;
    const originalModeHandler = UI.modeBtn?.onclick || null;
    const originalLrcToggleHandler = UI.lrcToggleBtn?.onclick || null;
    const originalLrcSettingsHandler = UI.lrcSettingsBtn?.onclick || null;
    const originalLrcPlainHandler = UI.lrcModePlainBtn?.onclick || null;
    const originalLrcPopupHandler = UI.lrcModePopupBtn?.onclick || null;
    const originalLrcScrollHandler = UI.lrcModeScrollBtn?.onclick || null;
    const originalLrcFallHandler = UI.lrcModeFallBtn?.onclick || null;
    const originalLrcFontSliderHandler = UI.lrcFontSlider?.oninput || null;
    const originalLrcBottomHandler = UI.lrcBottomSlider?.oninput || null;
    const originalSourceHandler = UI.sourceSelect?.onchange || null;
    const originalAddButtonHandler = UI.addBtn?.onclick || null;
    const originalInputHandler = UI.input?.oninput || null;
    const originalShapeHandler = UI.shapeBtn?.onclick || null;
    const originalSizeHandler = UI.sizeSlider?.oninput || null;
    const originalBallVisibleHandler = UI.ballVisibleToggle?.onchange || null;
    const originalColorHandler = UI.colorPicker?.oninput || null;
    const originalDecorationUploadHandler = UI.decorationUploadInput?.onchange || null;
    const originalDecorationClearHandler = UI.decorationClearBtn?.onclick || null;
    const originalDecorationLabelHandler = UI.decorationLabelInput?.oninput || null;
    const originalBgUploadHandler = UI.bgUploadInput?.onchange || null;
    const originalBgClearHandler = UI.bgClearBtn?.onclick || null;
    const originalRatioHandler = UI.ratioBtn?.onclick || null;
    const originalBgBlurHandler = UI.bgBlurSlider?.oninput || null;
    const originalBgBrightnessHandler = UI.bgBrightnessSlider?.oninput || null;

    // 第一次加载时当前播放器已经完成 DOM/状态构建，但保持休眠；Entry 随后调用 open。
    STATE.isExpanded = false;
    STATE.runtimeSleeping = true;
    setPlayerRuntimeSleeping(true);
    if (UI.panel) UI.panel.classList.remove('open');

    targetWin.__APV_PLAYER_RUNTIME__ = {
        open() { if (runtimeSleeping) setPlayerRuntimeSleeping(false); if (!STATE.isExpanded) togglePanel(); },
        close() { if (STATE.isExpanded) togglePanel(); },
        toggle() { if (STATE.isExpanded) togglePanel(); else { if (runtimeSleeping) setPlayerRuntimeSleeping(false); togglePanel(); } },
        sleep() { setPlayerRuntimeSleeping(true); },
        wake() { setPlayerRuntimeSleeping(false); },
        isPlaying() { return !!STATE.isPlaying; },
        destroy() { try { audio?.pause(); audio.src=''; } catch(_){}; try { targetDoc.getElementById(CONFIG.ID)?.remove(); }catch(_){}; delete targetWin.__APV_PLAYER_RUNTIME__; }
    };
    ...
    targetWin.addEventListener('pagehide', () => {
        ...
        if (c) c.remove();
        delete targetWin.__APV_PLAYER_RUNTIME__;
    }, { once: true });
